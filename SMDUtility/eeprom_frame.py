...
class eeprom_frame(LabelFrame):
	...
	def __init__(self,window,protocol,id):
		...
		self.gui_spacer("---")
		self.gui_entry("CPR Encoder", "cpr_encoder", 0, True, True, True, 0x23, 1, 2 )
		self.gui_entry("Reducer ratio Motor", "reducer_motor", 0, True, True, True, 0x25, 1, 2 )
		...
		self.gui_spacer("---")
		self.gui_entry("PID Velocity KP", "pid_position_kp", 0, True, True, True, 0x2A, 1, 2 )
		self.gui_entry("PID Velocity KI", "pid_position_ki", 0, True, True, True, 0x2C, 1, 2 )
		self.gui_entry("PID Velocity KD", "pid_position_kd", 0, True, True, True, 0x2E, 1, 2 )
		...
		self.gui_entry("Voltage Sense K", "cal_voltage_sense_k", 0, True, True, True, 0x3C, 1, 2 )
	...
	def read_all(self):
		...
		if error != 0 :
			...
			self.variables['cpr_encoder_local'].set(str(result[35] + (result[36]<<8)))
			self.variables['cpr_encoder_servo'].set(str(result[35] + (result[36]<<8)))
			self.variables['reducer_motor_local'].set(str(result[37] + (result[38]<<8)))
			self.variables['reducer_motor_servo'].set(str(result[37] + (result[38]<<8)))
			...
			self.variables['cal_voltage_sense_k_local'].set(str(result[60] + (result[61]<<8)))
			self.variables['cal_voltage_sense_k_servo'].set(str(result[60] + (result[61]<<8)))
